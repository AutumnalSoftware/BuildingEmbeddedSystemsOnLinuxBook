...
RunLoops::RunLoops(
    ...
    moodycamel::ReaderWriterQueue<weather::FusedWeatherSample, 64>& fusedQ,
...
void RunLoops::consumer(const std::atomic<bool>& stop)
{
    ...
    while (!stop.load(std::memory_order_relaxed))
    {
        if (m_inQ.try_dequeue(msg))
        {
            ...
            weather::FusedWeatherSample fused;
            if (m_fusion.accept(msg, fused))
            {
                if (m_fusedQ.try_enqueue(fused))
                {
                    m_stats.fusedEnqOk.fetch_add(1, std::memory_order_relaxed);
                }
                else
                {
                    m_stats.fusedDrops.fetch_add(1, std::memory_order_relaxed);
                }
            }
        }
        else
...
void RunLoops::logger(const std::atomic<bool>& stop)
{
    ...
    std::uint64_t lastFusedEnqOk = 0;
    std::uint64_t lastFusedDeq   = 0;
    std::uint64_t lastFusedDrops = 0;
    ...
    while (!stop.load(std::memory_order_relaxed))
    {
        ...
        if (now - lastReport >= seconds(1))
        {
            ...
            const std::uint64_t fusedEnqOk =
                m_stats.fusedEnqOk.load(std::memory_order_relaxed);
            const std::uint64_t fusedDeq =
                m_stats.fusedDeq.load(std::memory_order_relaxed);
            const std::uint64_t fusedDrops =
                m_stats.fusedDrops.load(std::memory_order_relaxed);
            ...
            const std::uint64_t fusedEnqPerSec   = fusedEnqOk   - lastFusedEnqOk;
            const std::uint64_t fusedDeqPerSec   = fusedDeq     - lastFusedDeq;
            const std::uint64_t fusedDropsPerSec = fusedDrops   - lastFusedDrops;

            lastFusedEnqOk = fusedEnqOk;
            lastFusedDeq   = fusedDeq;
            lastFusedDrops = fusedDrops;
            ...
            std::cout
                ...
                << "fused(enq=" << fusedEnqPerSec << "/s deq=" << fusedDeqPerSec
                << "/s fdrops=" << fusedDrops << " (+" << fusedDropsPerSec << "/s)) "
                << "q(depth=" << depth << " hi=" << hi << ") "
...
void RunLoops::output(const std::atomic<bool>& stop)
{
    using namespace std::chrono;

    weather::FusedWeatherSample sample{};

    while (!stop.load(std::memory_order_relaxed))
    {
        if (m_fusedQ.try_dequeue(sample))
        {
            m_stats.fusedDeq.fetch_add(1, std::memory_order_relaxed);
            std::cout << sample << "\n";
        }
        else
        {
